...
const map = new Map<string, Minesweeper>();
...
export function setPlayerMap(id: string, board: Minesweeper) {
	map.set(id, board);
}

export default class Minesweeper extends Page<
	{},
	{
		board: Cell[][];
...
> {
	...
	constructor() {
		...
		const board = Minesweeper.generateBoard(6, 6, 4);
		this.state = {
			board,
				...
				return row
					.map((cell, cellIndex) => {
						if (
							...
							return CellTypes.HIGHLIGHTED;
						}
						if (cell.revealed || ended) {
							return cell.type;
						}
						if (cell.flagged) {
